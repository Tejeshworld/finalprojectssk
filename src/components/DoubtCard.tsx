...
export default function DoubtCard({ doubt: initialDoubt, onUnbookmark }: { doubt: any, onUnbookmark?: (id: string) => void }) {
  ...
  // Safe links hydration string vs array support
  let links: string[] = [];
  if (Array.isArray(doubt.links)) {
    links = doubt.links;
  } else if (typeof doubt.links === "string") {
    try {
      links = JSON.parse(doubt.links);
    } catch {
      links = [doubt.links];
    }
  }
