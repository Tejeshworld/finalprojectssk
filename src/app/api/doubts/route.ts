...
export async function GET(request: Request) {
  try {
    ...
    const doubtsWithUserState = doubts.map(d => {
      const { reactions, bookmarks, images, links, ...rest } = d;
      return {
        ...rest,
...
export async function POST(request: Request) {
  try {
    ...
    const { title, description, codeSnippet, hubId, difficulty, priority, whiteboardImage, images, links } = await request.json();
